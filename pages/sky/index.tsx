...
import { Environment, OrbitControls, useAnimations,Html, Stats, TransformControls,PerspectiveCamera, PivotControls, useHelper, Point, Sky } from "@react-three/drei";
...
const handleWheel = (e:any) => {
  ...
  let skyvar = {t:10,d:1000,i:0.6,a:0.1,mc:0.005,md:0.8,r:0.5}
//gui
    const gui = new Gui.GUI()
    gui.add(skyvar,"t").min(0).max(100).step(1).name("turbidity")
    gui.add(skyvar,"d").min(0).max(1000).step(1).name("distance")
    gui.add(skyvar,"i").min(-2).max(2).step(0.1).name("inclination")
    gui.add(skyvar,"a").min(-2).max(2).step(0.1).name("azimuth")
    gui.add(skyvar,"md").min(0).max(3).step(0.1).name("mieD")
    gui.add(skyvar,"mc").min(0).max(0.2).step(0.0001).name("mieC")
    gui.add(skyvar,"r").min(0).max(10).step(0.1).name("rayleigh")
...
//gui
  ...
  useFrame((state, delta) => {
    ...
    skyref.current.material.uniforms.distance = skyvar.d
    skyref.current.material.uniforms.incination = skyvar.i
    skyref.current.material.uniforms.azimuth = skyvar.a
    skyref.current.material.uniforms.turbidity.value = skyvar.t
    skyref.current.material.uniforms.mieCoefficient.value = skyvar.mc
    skyref.current.material.uniforms.mieDirectionalG.value = skyvar.md
    skyref.current.material.uniforms.rayleigh.value = skyvar.r
  ...
  return(
  <>
  <Sky ref={skyref} turbidity={100} sunPosition={[0,0,10]} distance={600} inclination={1} azimuth={1} mieCoefficient={0.001} mieDirectionalG={1} rayleigh={2}  />
