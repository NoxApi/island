...
export default function Home() {
  ...
  const getsavedo = async () => {
    ...
    // set it to state
    // console.log(result)
    setsavedo(result[0]._document.data.value.mapValue.fields)
  ...
  const update = async () => {
    try {
      setsavedo(null)
      updateDoc(doc(firestore,'object',"build"),{
...
//gui
    const gui = new Gui.GUI()
        gui.add(safevalue,"x").min((safevalue.x)-10).max((safevalue.x+10)).step(0.1).name("obj pos-x")
        gui.add(safevalue,"y").min((safevalue.y)-10).max(safevalue.y+10).step(0.1).name("obj pos-y")
        gui.add(safevalue,"z").min((safevalue.z)-10).max(safevalue.z+10).step(0.1).name("obj pos-z")
...
//gui
 ...
 return () =>{ window.removeEventListener("wheel", handleWheel);
  ...
  useFrame((state, delta) => {
    ...
    refgroup.current!.position.copy(new THREE.Vector3(safevalue.x,safevalue.y,safevalue.z))
    refgroup.current.rotation.x = (Math.PI/180)*safevalue.rx
    refgroup.current.rotation.y = (Math.PI/180)*safevalue.ry
    refgroup.current.rotation.z = (Math.PI/180)*safevalue.rz
    refgroup.current.scale.x = safevalue.s
    refgroup.current.scale.y = safevalue.s
    refgroup.current.scale.z = safevalue.s
